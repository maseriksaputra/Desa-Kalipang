<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <form action="{{ route('admin.reports.update-status', $report->id) }}" method="POST" class="flex items-center gap-4">
                        ...
                        <div class="flex-grow">
                            <select name="status" class="block w-full border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:focus:border-indigo-400 rounded-md shadow-sm transition-colors">
                                ...
                                <option value="selesai" {{ $report->status == 'selesai' ? 'selected' : '' }}>Selesai (Final)</option>
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                                Kategori: {{ $report->category ?? 'Tidak ada' }}
                            </span>
                        </div>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Dilaporkan oleh <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $report->user->name }}</span> ({{ $report->user->email }}) pada {{ $report->created_at->format('d M Y, H:i') }}</p>
                    </div>
                </div>
            ...
            <!-- Bagian Penugasan (Assign to Pegawai) -->
            <div class="bg-white dark:bg-slate-800/80 dark:backdrop-blur-xl overflow-hidden shadow-sm dark:shadow-2xl sm:rounded-2xl border border-gray-100 dark:border-slate-700/50 transition-all duration-300">
                <div class="border-b border-gray-200 dark:border-slate-700/50 px-6 py-4 bg-gray-50 dark:bg-slate-900/50 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Teruskan Laporan ke Pegawai (Disposisi)</h3>
                </div>
                <div class="p-6">
                    <form action="{{ route('admin.reports.assign', $report->id) }}" method="POST" class="flex items-center gap-4">
                        @csrf
                        @method('PATCH')
                        <div class="flex-grow">
                            <select name="assigned_to" class="block w-full border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:focus:border-indigo-400 rounded-md shadow-sm transition-colors">
                                <option value="">-- Belum Ditugaskan --</option>
                                @foreach($pegawai as $p)
                                    <option value="{{ $p->id }}" {{ $report->assigned_to == $p->id ? 'selected' : '' }}>
                                        {{ $p->name }} - Bidang: {{ $p->department }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <x-primary-button>Tugaskan / Teruskan</x-primary-button>
                    </form>
                </div>
            </div>
