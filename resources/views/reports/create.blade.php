            ...
            <div class="bg-white dark:bg-slate-800/80 dark:backdrop-blur-xl overflow-hidden shadow-sm dark:shadow-2xl sm:rounded-2xl border border-gray-100 dark:border-slate-700/50 transition-all duration-300">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('reports.store') }}" enctype="multipart/form-data">
                        ...
                        <!-- Category -->
                        <div class="mb-6">
                            <x-input-label for="category" :value="__('Kategori Laporan')" />
                            <select id="category" name="category" class="block mt-1 w-full border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:focus:border-indigo-400 rounded-md shadow-sm transition-colors" required>
                                <option value="" disabled selected>Pilih Kategori</option>
                                <option value="Infrastruktur" {{ old('category') == 'Infrastruktur' ? 'selected' : '' }}>Infrastruktur (Jalan, Jembatan, dll)</option>
                                <option value="Sosial" {{ old('category') == 'Sosial' ? 'selected' : '' }}>Sosial (Bantuan, Kesejahteraan)</option>
                                <option value="Kesehatan" {{ old('category') == 'Kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                                <option value="Keamanan" {{ old('category') == 'Keamanan' ? 'selected' : '' }}>Keamanan & Ketertiban</option>
                                <option value="Layanan Publik" {{ old('category') == 'Layanan Publik' ? 'selected' : '' }}>Layanan Publik (Administrasi)</option>
